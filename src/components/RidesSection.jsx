...
import rideCategories from "../assets/image.png";
...
const RidesSection = () => {
  ...
  const getVisibleRides = () => {
    const visibleRides = [];
    for (let i = 0; i < visibleCards; i++) {
      visibleRides.push(filteredRides[(currentIndex + i) % filteredRides.length]);
    }
    return visibleRides;
  };
      ...
      <section className="text-white py-4">
        ...
        <div className="relative flex flex-col md:flex-row items-center mt-">
          ...
          <div className="flex-1 ml-auto flex justify-end">
            <div className="overflow-hidden relative" style={{ width: `${cardWidth * visibleCards + 3 * (visibleCards - 1)}px` }}> {/* Set width */}
              <div
                className="flex transition-transform duration-1000 ease-in-out gap-[3px]"
                style={{
                  transform: `translateX(-${currentIndex * (cardWidth + 3)}px)`,
                  width: `${filteredRides.length * (cardWidth + 3)}px`, // Required width for the loop
                }}
              >
